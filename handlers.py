...
def register_all_handlers(bot: TeleBot):
    """ثبت تمامی هندلرهای اصلی، FSM، ویرایش فیلدها، ثبت معامله، کیف پول و Callbacks ربات می‌انجی"""
    ...
    # ====================================================
    # ۳. انتخاب حوزه کاری و راهنمای فرم ثبت (بدون خط حوزه در متن نمونه)
    # ====================================================
    ...
    def handle_category_button(message: Message):
        ...
        # خط حوزه از متن نمونه کلاً حذف گردید تا حوزه انتخاب‌شده توسط دکمه حفظ شود
        sample_text = (
    ...
    # ====================================================
    # ۵. هندلر اختصاصی دکمه «رد کردن و استفاده از شماره تلگرام» (اصلاح‌شده)
    # ====================================================
    ...
    # ====================================================
    # ۷. دانلود PDF و اکشن‌های قرارداد
    # ====================================================
    ...
    @bot.callback_query_handler(func=lambda call: call.data.startswith(("sign_contract_", "bargain_", "cancel_", "deliver_", "confirm_")))
    def handle_contract_actions(call: CallbackQuery):
        action_parts = call.data.split("_")
        action = action_parts[0]
        cid = "_".join(action_parts[1:])
        user_id = call.from_user.id
        contract = db.get_contract(cid)
        ...
        if action in ["sign", "confirm"]:
            db.update_contract(cid, {"status": "active"})
            bot.answer_callback_query(call.id, "✅ معامله تایید و فعال شد.")
            bot.send_message(call.message.chat.id, f"🎉 معامله شماره `{cid}` با موفقیت امضا و وارد مرحله اجرا شد.", parse_mode="Markdown")

        elif action == "bargain":
            user_states[user_id] = {"bargain_cid": cid, "step": "WAITING_FOR_BARGAIN_PRICE"}
            bot.answer_callback_query(call.id)
            bot.send_message(call.message.chat.id, "💬 **لطفاً مبلغ پیشنهادی جدید خود را به تومان ارسال کنید:**")

        elif action == "cancel":
            db.update_contract(cid, {"status": "cancelled"})
            bot.answer_callback_query(call.id, "❌ معامله لغو شد.")
            bot.send_message(call.message.chat.id, f"🚫 معامله شماره `{cid}` لغو شد.")

        elif action == "deliver":
            db.update_contract(cid, {"status": "delivered"})
            bot.answer_callback_query(call.id, "🚀 کار تحویل داده شد.")
            bot.send_message(call.message.chat.id, f"📦 **پروژه معامله `{cid}` تحویل داده شد.** کارفرما باید تایید نهایی را انجام دهد.")

    # ====================================================
    # ۸. پردازش کلیه متون ارسالی کاربران
    # ====================================================
    ...
    def handle_text_messages(message: Message):
        ...
        # ۱. پردازش ویرایش مجزای فیلد پیش‌نویس (FSM)
        if state.get("step") == "WAITING_FOR_FIELD_EDIT":
            _, db_data = db.get_user_state(user_id)
        ...
        # ۲. پردازش چانه‌زنی و مبلغ جدید
        if state.get("step") == "WAITING_FOR_BARGAIN_PRICE":
        ...
        # ۳. پردازش فرم یکجای ثبت معامله و اعمال صحیح حوزه انتخابی
        parsed = utils.parse_single_message_contract(text)
        if parsed:
            # دریافت حوزه انتخابی از حافظه موقت یا دیتابیس (جلوگیری از اورراید به گرافیک)
            _, db_data = db.get_user_state(user_id)
...
def finalize_and_create_contract(bot: TeleBot, chat_id: int, user_id: int, draft_data: dict, phone_number: str):
    """ثبت رسمی معامله در دیتابیس و تولید لینک دعوت اختصاصی"""
    try:
        category = draft_data.get("category", "GEN")
        
        # تولید مطمئن شناسه معامله جهت جلوگیری از نال بودن ستون دیتابیس
        ...
        # پشتیبانی از بازگشت مستقیم یا موفقیت خام
        ...
        invite_link = f"https://t.me/{bot_username}?start=c_{contract_id}"
